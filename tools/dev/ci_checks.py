...
def workflow_policy() -> int:
    ...
    for path in sorted(WORKFLOW_DIR.glob("*.yml")):
        ...
        if "write-all" in text or "continue-on-error" in text:
            return fail(f"{path.relative_to(ROOT)} contains an unsafe permission or failure override")
        ...
        for line in text.splitlines():
            if "uses:" in line and not ACTION_REF.search(line):
                return fail(f"{path.relative_to(ROOT)} has an action without an exact version: {line.strip()}")
...
def terraform_policy() -> int:
    ...
    print(f"PASS terraform-policy: {len(files)} HCL files, exact provider pins, no resources or credentials")
    return 0
...
def schema_check() -> int:
    ...
    try:
        ...
        vectors = sorted((ROOT / "packages" / "schemas" / "canonical" / "vectors").glob("fixture_*.json"))
        if not vectors:
...
def openapi_check() -> int:
    candidates = sorted(
        ...
        if path.is_file() and path.name.lower().startswith("openapi") and path.suffix.lower() == ".json"
    )
    if candidates:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        document = {"openapi": "3.1.0", "info": {"title": "fixture", "version": "0.1.0"}, "paths": {}}
        ...
        candidate.write_text(json.dumps({**document, "info": {"title": "changed", "version": "0.1.0"}}), encoding="utf-8")
        if not module.compare(baseline, candidate):
            return fail("OpenAPI compatibility tool accepted a changed document")
    print("PASS openapi: compatibility gate rejects changes and accepts identical OpenAPI 3.1 fixtures")
    return 0


def license_policy() -> int:
    files = [ROOT / "package.json", ROOT / "pnpm-lock.yaml", ROOT / "pyproject.toml", ROOT / "uv.lock"]
    forbidden = ("GPL-3.0", "AGPL-3.0", "GPLv3", "AGPLv3")
    findings = [f"{path.relative_to(ROOT)} contains {license}" for path in files if path.exists() for license in forbidden if license in path.read_text(encoding="utf-8")]
    if findings:
        return fail("forbidden dependency licenses detected:\n" + "\n".join(findings))
    print("PASS license-policy: dependency manifests contain no denied GPL/AGPL license identifiers")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("check", choices=("workflow-policy", "terraform-policy", "docs-links", "schema", "openapi", "license-policy"))
